...
public class ReceptionistUI {
    ...
    private void viewAllVisits() {
        ...
        while (it.hasNext()) {
            ...
            if (v instanceof AppointmentVisit) {
                displayDate = ((AppointmentVisit) v).getSlot().toLocalDate();  // Appointment date
            } else {
                displayDate = v.getTime().toLocalDate();  // Walk-in registration date
            }
            System.out.printf("%-10s | %-20s | %-10s | %-20s | %-17s | %-10s | %s%n",
                v.getVisitID(),
    ...
    private void viewVisitsByDate() {
        ...
        while (it.hasNext()) {
            ...
            System.out.printf("%-10s | %-20s | %-10s | %-20s | %-17s | %-21s | %s%n",
                v.getVisitID(),
                OutputHelper.truncate(v.getPatient().getName(), 20),
                type,
                OutputHelper.truncate(v.getService().getServiceName(), 20),
